...
class DqlTfFlexible(LearningModel):
    ...
    def __init__(self, action_wrapper: ActionWrapper, state_builder: StateBuilder, save_path='urnai/models/saved/', file_name='temp', learning_rate=0.0002, gamma=0.95, name='DQN', build_model = DqlTfFlexible.DEFAULT_BUILD_MODEL):
    ...
    def make_model(self, build_model):
        if build_model == DqlTfFlexible.DEFAULT_BUILD_MODEL:
                    build_model[0]['shape'] = [None, self.state_size]
                    build_model[3]['length'] = self.actions_size

        self.model_layers = []
        for layer_model in build_model:
            if layer_model['type'] == LAYER_INPUT: 
                if build_model.index(layer_model) == 0:
